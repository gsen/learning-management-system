...
export default function InstructorDashboardPage() {
  ...
  if (error) {
    return <p className="text-sm text-destructive">{getApiErrorMessage(error, "Unable to fetch dashboard.")}</p>;
  }
    ...
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Review created courses and total student counts.</p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Card size="sm">
          <CardHeader>
            ...
            <CardTitle className="text-3xl">{instructorCourses.length}</CardTitle>
          </CardHeader>
        </Card>
        <Card size="sm">
          <CardHeader>
        ...
        <Card size="sm">
          <CardHeader>
            ...
            <CardTitle className="line-clamp-1">{trendingCourse?.title || "No courses yet"}</CardTitle>
          </CardHeader>
        </Card>
      </section>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage course details and add lectures.</CardDescription>
          <CardAction>
            <Link to="/instructor/manage" className="text-sm underline underline-offset-4">
              Manage all
            </Link>
          </CardAction>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-2">
          {instructorCourses.length === 0 ? (
            <p className="text-sm text-muted-foreground md:col-span-2">Created courses will appear here.</p>
          ) : (
            instructorCourses.slice(0, 4).map((course) => (
              <div key={course._id} className="grid gap-3 rounded-2xl border p-3">
                ...
                <div className="grid gap-1">
                  ...
                  <p className="text-sm text-muted-foreground">{course.totalStudents || 0} students</p>
                </div>
