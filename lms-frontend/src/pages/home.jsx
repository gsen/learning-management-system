...
export default function HomePage() {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return allCourses.filter((course) => {
      ...
      return `${course.title || ""} ${course.description || ""} ${instructorName}`.toLowerCase().includes(term);
    });
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {isLoading &&
          Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="grid gap-3 rounded-4xl border bg-card p-4">
        ...
        {!isLoading && !error && allCourses.length > 0 && filteredCourses.length === 0 && (
          <p className="text-sm text-muted-foreground md:col-span-3">No courses match your search.</p>
        )}

        {!isLoading && !error && filteredCourses.map((course) => <CourseCard key={course._id} course={course} />)}
      </section>
    </div>
  );
}
