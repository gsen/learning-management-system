...
export default function ManageCoursesPage() {
  ...
  const [updateLecture, { isLoading: isUpdatingLecture }] = useUpdateLectureMutation();
  ...
  } = useFetchLecturesByCourseQuery(managingLectureCourse?._id, { skip: !managingLectureCourse });
  ...
  async function handleUpdateCourse(event) {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(updateError, "Unable to update course."), { position: "bottom-center" });
    }
  }
  ...
  async function handleCreateLecture(event) {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(createError, "Unable to add lecture."), { position: "bottom-center" });
    }
  }
  ...
  async function handleUpdateLecture(event) {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(updateError, "Unable to update lecture."), { position: "bottom-center" });
    }
  }
  ...
  async function handleDeleteLecture() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(deleteError, "Unable to delete lecture."), { position: "bottom-center" });
    }
  }
  ...
  async function handleDeleteCourse() {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(deleteError, "Unable to delete course."), { position: "bottom-center" });
    }
  }
  ...
  if (error) {
    return <p className="text-sm text-destructive">{getApiErrorMessage(error, "Unable to fetch courses.")}</p>;
  }
  ...
  return (
    <>
      <Card>
        ...
        <CardContent>
          {instructorCourses.length === 0 ? (
            <p className="text-sm text-muted-foreground">Created courses will appear here.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableRow key={course._id}>
                    <TableCell>
                      <div className="grid gap-1">
                        ...
                        <span className="line-clamp-1 text-muted-foreground">{course.description}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-2">
                        <Button size="icon-sm" variant="secondary" onClick={() => setLectureCourse(course)}>
                          <Plus />
                          <span className="sr-only">Add lecture</span>
                        </Button>
                        <Button size="icon-sm" variant="secondary" onClick={() => setManagingLectureCourse(course)}>
                          <ListVideo />
                          <span className="sr-only">Manage lectures</span>
                        </Button>
                        <Button size="icon-sm" variant="outline" onClick={() => setEditingCourse(course)}>
                          <Edit />
                          <span className="sr-only">Edit</span>
                        </Button>
                        <Button size="icon-sm" variant="destructive" onClick={() => setDeletingCourse(course)}>
                          <Trash2 />
      ...
      <Dialog open={Boolean(editingCourse)} onOpenChange={(open) => !open && setEditingCourse(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Update the course details students see in the catalog.</DialogDescription>
          </DialogHeader>
          {editingCourse && (
      ...
      <Dialog open={Boolean(lectureCourse)} onOpenChange={(open) => !open && setLectureCourse(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add lecture</DialogTitle>
            <DialogDescription>Add a video lecture to {lectureCourse?.title}.</DialogDescription>
          </DialogHeader>
          {lectureCourse && (
            <form className="grid gap-4" onSubmit={handleCreateLecture}>
              <FieldGroup>
                ...
                <Field>
                  <FieldLabel htmlFor="lecture-description">Description</FieldLabel>
                  <Textarea id="lecture-description" name="description" required />
                </Field>
                <Field>
                ...
                <Field>
                  <FieldLabel htmlFor="lecture-duration">Duration in minutes</FieldLabel>
                  <Input id="lecture-duration" name="duration" type="number" min="0" required />
                </Field>
              </FieldGroup>
              <DialogFooter>
      ...
      >
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Edit or remove lectures in {managingLectureCourse?.title}.</DialogDescription>
          </DialogHeader>
          {isLoadingLectures ? (
            <Skeleton className="h-48" />
          ) : lecturesError ? (
            <p className="text-sm text-destructive">{getApiErrorMessage(lecturesError, "Unable to fetch lectures.")}</p>
          ) : lectures.length === 0 ? (
            <div className="grid gap-3 rounded-2xl border p-4">
              <p className="text-sm text-muted-foreground">No lectures have been added yet.</p>
              <Button
                type="button"
                className="w-fit"
                onClick={() => {
                  ...
                  <TableRow key={lecture._id}>
                    <TableCell>
                      <div className="grid gap-1">
                        ...
                        <span className="line-clamp-1 text-muted-foreground">{lecture.description}</span>
                      </div>
                    </TableCell>
                    <TableCell>{lecture.duration || 0} min</TableCell>
                    <TableCell>
                      <div className="flex justify-end gap-2">
                        <Button size="icon-sm" variant="outline" onClick={() => setEditingLecture(lecture)}>
                          <Edit />
                          <span className="sr-only">Edit lecture</span>
                        </Button>
                        <Button size="icon-sm" variant="destructive" onClick={() => setDeletingLecture(lecture)}>
                          <Trash2 />
      ...
      <Dialog open={Boolean(editingLecture)} onOpenChange={(open) => !open && setEditingLecture(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit lecture</DialogTitle>
            <DialogDescription>Update lecture details and video metadata.</DialogDescription>
          </DialogHeader>
          {editingLecture && (
            <form className="grid gap-4" onSubmit={handleUpdateLecture}>
              <FieldGroup>
                <Field>
                  ...
                  <Input id="edit-lecture-title" name="title" defaultValue={editingLecture.title} required />
                </Field>
                <Field>
                  <FieldLabel htmlFor="edit-lecture-description">Description</FieldLabel>
                  <Textarea
                ...
                <Field>
                  <FieldLabel htmlFor="edit-lecture-video">Video URL</FieldLabel>
                  <Input id="edit-lecture-video" name="videoUrl" defaultValue={editingLecture.videoUrl} required />
                </Field>
                <Field>
                  <FieldLabel htmlFor="edit-lecture-duration">Duration in minutes</FieldLabel>
                  <Input
      ...
      <AlertDialog open={Boolean(deletingCourse)} onOpenChange={(open) => !open && setDeletingCourse(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove {deletingCourse?.title} and its lectures. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={Boolean(deletingLecture)} onOpenChange={(open) => !open && setDeletingLecture(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this lecture?</AlertDialogTitle>
            <AlertDialogDescription>
              This will remove {deletingLecture?.title} from the course. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeletingLecture}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteLecture} disabled={isDeletingLecture}>
