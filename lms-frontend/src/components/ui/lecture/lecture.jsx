        ...
        {lectures.map((lecture) => (
          <TableRow key={lecture._id}>
            ...
            <TableCell className="text-right">
              <Link to={`/player/${lecture.course}?lectureId=${lecture._id}`} className="text-blue-600 hover:underline">
                Preview
              </Link>
            </TableCell>
            <TableCell className="text-right">{formatDuration(lecture.duration)}</TableCell>
          </TableRow>
        ))}
      </TableBody>
      <TableFooter>
        <TableRow>
          ...
          <TableCell className="text-right">{formatDuration(getTotalDuration())}</TableCell>
        </TableRow>
      </TableFooter>
    </Table>
  );
